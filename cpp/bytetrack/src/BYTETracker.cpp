...
BYTETracker::BYTETracker(int   max_time_lost,
...
void BYTETracker::update(const std::vector<Object> & objects,
    ...
    if (objects.size() > 0) {
        for (int i = 0; i < objects.size(); i++) {
            ...
            strack.distance_ = objects[i].distance;
    ...
    linear_assignment(dists, dist_size, dist_size_size, match_thresh, matches, u_track,
                      u_detection);
